...
document.addEventListener('DOMContentLoaded', () => {
  const mapContainerId = 'travel-map';

  // Poll for the container in case it's rendered asynchronously
  const waitForContainer = setInterval(() => {
    const mapContainer = document.getElementById(mapContainerId);

    if (mapContainer) {
      clearInterval(waitForContainer); // stop polling
      console.log("Map container found. Initializing map...");

      // Initialize Leaflet map
      const map = L.map(mapContainerId).setView([40.7128, -74.0060], 2);
      ...
      console.log("Map initialized with all markers.");
    } else {
